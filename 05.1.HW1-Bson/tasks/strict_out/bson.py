...
def E_list(data: Dict[Any, Any], data_for: Dict[Any, Any]) -> bytes:
    e_list = b''
    for key in data:
        elem = data[key]
        e_list += Element(key, elem, data_for)
    return e_list
...
def Element(key: str, elem: Any, data_for: Dict[Any, Any]) -> bytes:
    e_name = Cstring(key)
    if isinstance(elem, str):
